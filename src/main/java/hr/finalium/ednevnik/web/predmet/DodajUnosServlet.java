 ...
 *
 */
@WebServlet("/servleti/predmet/dodajUnos")
public class DodajUnosServlet extends HttpServlet {
	/***/
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		req.setAttribute("kategorije", predmet.getKategorije().parallelStream()
				.filter(kat -> !kat.getNaziv().equals(Konstante.ZAKLJUCNA_OCJENA)).collect(Collectors.toSet()));
		req.setAttribute("razred", razred);

		req.getRequestDispatcher("/WEB-INF/profesor/dodajUnos.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Kategorija kategorija = predmet.getKategorije().parallelStream()
				.filter(kat -> kat.getNaziv().equals(req.getParameter("kategorija"))).findAny().get();
		...
		resp.sendRedirect(
				"/ednevnik/servleti/predmet/dodajUnos?idPredmet=" + idPredmet + "&idRazred=" + u.getRazred().getId());
	}
}
